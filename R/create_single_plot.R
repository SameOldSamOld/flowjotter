...
create_single_plot <- function(
  ...
  if (substr(label, 1, 1) == "%") {
    ...
    df            <- data.frame(`x` = stats::rnorm(20), `y` = stats::rnorm(20))
    g_output      <- ggplot2::ggplot(df, ggplot2::aes(x,y)) +
      ggplot2::geom_blank()
  }
  ...
  if (colours == "Black & White") {
    ...
    cols = grDevices::colorRampPalette(
      RColorBrewer::brewer.pal(8, colours))(length(unique(tempData$Samples)))
